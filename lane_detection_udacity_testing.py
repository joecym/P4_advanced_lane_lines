...
def pipeline(img, s_thresh=(170, 255), sx_thresh=(20, 100)):
    ...
    plt.figure(fname)
    plt.imshow(combined)
    plt.savefig('/home/rrc/Desktop/self-drive/short/color/output_images/tranform'+str(n)+'.jpg')
...
def unwarp_lane(img, mtx, dist):
    ...
    # output here
    plt.figure(fname)
    plt.imshow(undist_orig)
    plt.savefig('/home/rrc/Desktop/self-drive/short/color/output_images/seek'+str(n)+'.jpg')

    # output here
    plt.figure(fname)
    plt.imshow(warped)
    plt.savefig('/home/rrc/Desktop/self-drive/short/color/output_images/top_down'+str(n)+'.jpg')
...
def process_image(image):
    ...
    offset_near_left = 30  # define offset to check for large changes 
    offset_far_left = 100 
    offset_near_right = 30
    offset_far_right = 30
    ...
    try:
        if right.bestx is not None and left.bestx is not None:
            ...
            for tup in tupled_pix_left:
                ...
                tupled_pix_left_lane.append(temp)
            

            tupled_pix_right_lane = []
            for tup in tupled_pix_right:
                ...
                tupled_pix_right_lane.append(temp)
            print tupled_pix_left_lane
            return tupled_pix_left_lane, tupled_pix_right_lane
...
images = glob.glob('/home/rrc/Desktop/self-drive/short/color/*.jpg')
n = 0
...
for fname in images:
    ...
    plt.figure(fname)
    plt.imshow(image)
    plt.savefig('/home/rrc/Desktop/self-drive/short/color/output_images/warped'+str(n)+'.jpg')
    l_scatter, r_scatter = pub_scatter(left, right)
    print np.shape(l_scatter), np.shape(r_scatter)
    n = n + 1
