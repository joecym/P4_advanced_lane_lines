...
pub_image_transform = rospy.Publisher('lane_detection_image_transform',Image,queue_size=1)
pub_point_cloud_left = rospy.Publisher('lane_detection_point_cloud',PointCloud,queue_size=1)
pub_point_cloud_right = rospy.Publisher('lane_detection_point_cloud',PointCloud,queue_size=1)
...
def pipeline(img, s_thresh=(170, 255), sx_thresh=(20, 100)):
    ...
    img = bridge.cv2_to_imgmsg(combined)
    pub_image_transform.publish(img)
...
def unwarp_lane(img, mtx, dist):
    ...
    img = bridge.cv2_to_imgmsg(undist_orig)
    ...
    img = bridge.cv2_to_imgmsg(warped)
...
def callback(data):
	...
	pub_point_cloud_left.publish(left_points)
	pub_point_cloud_right.publish(right_points)
